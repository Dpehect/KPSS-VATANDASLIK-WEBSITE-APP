...
const levelDescriptions = {
  ...
  zor: "Seçici yorum, anayasal süreç, çeldirici ve sık karıştırılan kavramlara odaklanır.",
} as const;
...
export async function TopicQuestionPage({ topicId, testId, level }: { topicId: string; testId?: string; level?: TestLevel }) {
  ...
  const title = topic ? `${topic.title} testleri` : "Karışık KPSS Vatandaşlık testleri";
  if (level) {
    return <main className="space-y-8"><QuestionHeader eyebrow="Konu Tarama Testi" title={`${title} · ${levelTitles[level]}`} description={levelDescriptions[level]} backHref={`/question-bank/${topicId}`} backLabel="Seviye seçimine dön" icon={isMixed ? <Shuffle size={28} /> : <FileQuestion size={28} />} />{isMixed ? <MixedTopicsNotice topics={topics} /> : null}<div className="grid gap-4 lg:grid-cols-2">{visibleTests.map((test) => <TestCard key={test.id} topicId={topicId} test={test} isMixed={isMixed} />)}</div></main>;
  }

  return <main className="space-y-8"><QuestionHeader eyebrow="Soru bankası" title={title} description="Seviyenizi seçin ve gelişmiş altyapımız ile hazırlanan özgün testlere başlayın." backHref="/question-bank" backLabel="Soru bankasına dön" icon={isMixed ? <Shuffle size={28} /> : <FileQuestion size={28} />} />{isMixed ? <MixedTopicsNotice topics={topics} /> : null}<div className="grid gap-4 lg:grid-cols-3">{levelOrder.map((item) => <LevelChoiceCard key={item} topicId={topicId} level={item} isMixed={isMixed} tests={allTests.filter((test) => test.level === item)} />)}</div></main>;
}
